...
fn date_string<'s>(i: &mut &'s str) -> Result<&'s str> {
...
fn base_account_name<'s>(i: &mut &'s str) -> Result<&'s str> {
    alt((
        literal("Assets"),
        literal("Liabilities"),
        literal("Equity"),
...
fn account_name<'s>(i: &mut &'s str) -> Result<&'s str> {
    take_while(1.., |c: char| c.is_alphanumeric() || c == '-').parse_next(i)
}

fn subaccount<'s>(i: &mut &'s str) -> Result<()> {
    separated(1.., account_name, ":").parse_next(i)
}

fn full_account<'s>(i: &mut &'s str) -> Result<&'s str> {
    separated_pair(base_account_name, ":", subaccount)
        .take()
        .parse_next(i)
}

fn quoted_string<'s>(i: &mut &'s str) -> Result<&'s str> {
    delimited('"', take_while(1.., |c| c != '"'), '"').parse_next(i)
}

fn comment<'s>(i: &mut &'s str) -> Result<&'s str> {
    preceded(';', take_while(0.., |c: char| c != '\n' && c != '\r'))
        .take()
        .parse_next(i)
}

fn tag<'s>(i: &mut &'s str) -> Result<&'s str> {
    preceded('#', alphanumeric1).take().parse_next(i)
}

fn tag_list<'s>(i: &mut &'s str) -> Result<Vec<&'s str>> {
    separated(1.., tag, " ").parse_next(i)
}

fn decimal_string<'s>(i: &mut &'s str) -> Result<&'s str> {
    seq!(_: opt('-'),
     _: digit1,
...
fn commodity<'s>(i: &mut &'s str) -> Result<&'s str> {
    take_while(1.., |c: char| {
...
fn open_statement<'s>(i: &mut &'s str) -> Result<OpenParams<'s>> {
    seq!(OpenParams {
         date: date_string,
         _: space1,
         _: literal("open"),
...
fn close_statement<'s>(i: &mut &'s str) -> Result<CloseParams<'s>> {
    seq!(CloseParams {
         date: date_string,
         _: space1,
         _: literal("close"),
...
fn balance_statement<'s>(i: &mut &'s str) -> Result<BalanceParams<'s>> {
    seq!(BalanceParams {
         date: date_string,
         _: space1,
         _: literal("balance"),
...
fn include_statement<'s>(i: &mut &'s str) -> Result<IncludeParams<'s>> {
    seq!(IncludeParams {
         _: literal("include"),
         _: space1,
         path: quoted_string,
...
fn transaction_header<'s>(i: &mut &'s str) -> Result<&'s str> {
    seq!(_: date_string,
         _: space1,
         _: literal("*"),
         _: space1,
...
fn posting<'s>(i: &mut &'s str) -> Result<&'s str> {
    seq!(_: literal("  "),
         _: full_account,
         _: opt(seq!(_: space1,
                     _: decimal_string,
...
fn transaction_statement<'s>(i: &mut &'s str) -> Result<Transaction<'s>> {
    seq!(Transaction {
...
fn event_statement<'s>(i: &mut &'s str) -> Result<&'s str> {
    seq!(_: date_string,
         _: space1,
         _: literal("event"),
         _: space1,
...
fn option_statement<'s>(i: &mut &'s str) -> Result<&'s str> {
...
fn custom_statement<'s>(i: &mut &'s str) -> Result<&'s str> {
    seq!(_: date_string,
         _: space1,
         _: literal("custom"),
         _: till_line_ending)
...
fn comment_statement<'s>(i: &mut &'s str) -> Result<&'s str> {
    seq!(_: space0,
         _: comment)
    .take()
    .parse_next(i)
...
fn empty_statement<'s>(i: &mut &'s str) -> Result<&'s str> {
    space1.take().parse_next(i)
}

fn other_statement<'s>(i: &mut &'s str) -> Result<&'s str> {
    till_line_ending.parse_next(i)
}

fn active_statement<'s>(i: &mut &'s str) -> Result<Statement<'s>> {
    alt((
...
fn active_statements<'s>(i: &mut &'s str) -> Result<Vec<Statement<'s>>> {
    separated(0.., active_statement, line_ending).parse_next(i)
}

fn full_file<'s>(i: &mut &'s str) -> Result<Vec<Statement<'s>>> {
    let (active_statements, _) = (active_statements, eof).parse_next(i)?;
    Ok(active_statements)
}

pub fn parse_file<'s>(i: &mut &'s str) -> Result<Vec<Statement<'s>>> {
...
mod tests {
    ...
    #[test]
    fn test_narration() {
        let mut i = r#""hello blag ' again""#;
        let a = quoted_string.parse_next(&mut i).unwrap();
        let e = ("", "hello blag ' again");
        assert_eq!((i, a), e)
    }

    #[test]
    fn test_full_account() {
        let mut i = r#"Income:Income-Stan:Rental"#;
        let a = full_account.parse_next(&mut i).unwrap();
        let e = ("", "Income:Income-Stan:Rental");
        assert_eq!((i, a), e)
    }

    #[test]
    fn test_comment() {
        let mut i = ";this is a comment income:Income-Stan:Rental\n";
    ...
    fn test_open_statement() {
        let mut i = "2024-01-01 open  Expenses:Adjustment";
    ...
    fn test_close_statement() {
        let mut i = "2024-01-01 close Expenses:Adjustment";
    ...
    fn test_balance_statement() {
        let mut i = "2024-01-01 balance  Expenses:Adjustment                   0.00 CAD";
